 ...
 * Otherwise returns the next position in the queue to append to.
 */
int append_to_bbq(BBQ *bbq, sector_t addr, char *dev_name, ktime_t time) {
	
	/* minimal null pointer check */
	if (bbq == NULL) {
		return -1;
	}
	
	/* obtain address to next blob o' write info */
	BB *blob = &(bbq->queue[bbq->next_pos]);
	
	/* another minimal null pointer check */
	if (blob == NULL) {
		return -2;
	}
...
/* System Programming Fall 2020 END */
	...
	if (bio_has_data(bio)) {
		...
		if (rw & WRITE) {
			...
			if (bdevname(bio->bi_bdev, dev_name_buf) == NULL) {
				printk(KERN_ERR "ERROR: Device name empty after bdevname().\n");
			} else {
				append_to_bbq(&g_bbq,
			...
			char b[BDEVNAME_SIZE];
			printk("DEBUG: WRITE to block %u on %s (%u sectors) at time %u\n",
				bio->bi_iter.bi_sector,
				bdevname(bio->bi_bdev, b),
				count,
				ktime_get_real());
			
			printk("Next BBQ Place: %d. Yummy? %s\n",
				g_bbq.next_pos, yummy ? "yes." : "no.");
			*/
			...
			printk(KERN_DEBUG "G_BBQ: WRITE to block %u on %s (%u sectors) at time %u\n"
				...
				count,
				g_bbq.queue[pos].time,
				pos,
				yummy ? "Yummy." : "Not yummy.");
			//*/
			}
